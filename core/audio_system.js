...
class AudioManagerSystem {
    constructor() {
        ...
        // ── Synth SFX Gate Timestamps ───────────────────────────────────────
        // These gate the procedural (Web Audio API) sound effects.
        // The SfxManager (MP3 pool) gates are set separately via setSilence().
        this._sfxGateTime    = 0;   // Blocks ALL synth sfx until this timestamp
        this._combatGateTime = 0;   // Blocks combat synth sfx until this timestamp
        ...
        // ── MUSICAL SCALES ─────────────────────────────────────────────────
        const SCALES = {
            majorPentatonic: [0, 2, 4, 7, 9, 12, 14],
            minorPentatonic: [0, 3, 5, 7, 10, 12, 15],
            harmonicMinor:   [0, 2, 3, 5, 7, 8, 11, 12],
            hirajoshi:       [0, 2, 3, 7, 8, 12, 14],
            dorian:          [0, 2, 3, 5, 7, 9, 10, 12],
            chromatic:       [0, 1, 2, 3, 4, 5, 6, 7],
            drone:           [0, 7, 12, 0, 7, 12, 0]
        };
        ...
        this.tracks = {
            ...
            "Bandits": {
                scale: SCALES.chromatic, root: 55, tempo: 220, wave: 'sawtooth',
                pattern: [
                    0,3,1,7, 2,9,3,-1,
                    11,4,6,8, 10,-1,7,5,
                    12,10,8,6, 4,2,0,-1,
    ...
    init() {
        ...
        console.log('[AudioManager] Procedural Audio System Initialized');
    }
    ...
    playMP3(url, loop = true) {
        // Prevent double-play if the exact track is already active
        if (this.currentTrack === url && this.currentMp3 && !this.currentMp3.paused) {
            console.log('[AudioManager] Track already playing, skipping ->', url);
            return;
        }

        this.stopMusic(); // Stop synth
        this.stopMP3();   // Stop MP3 (increments session ID)
        ...
        if (loop) {
            newAudio.addEventListener('ended', function() {
                this.currentTime = 0;
                this.play().catch(e => console.warn('[AudioManager] Manual loop restart failed:', e));
            }, false);
        }

        newAudio.play().then(() => {
            // Race condition check: did another file stop/change the music while buffering?
            if (this._playSessionId !== currentSession) {
                this._nukeAudio(newAudio);
                return;
            }
            this._fadeMP3(newAudio, this.mp3Volume, 1000);
        }).catch(err => {
            console.warn('[AudioManager] Browser blocked audio or file not found:', err);
        });

        console.log('[AudioManager] Playing MP3:', url);
    }
    ...
    _playNextInList(previousTrack = null) {
        ...
        console.log('[AudioManager] Playlist playing:', nextTrackUrl);

        const currentAudio = new Audio(nextTrackUrl);
        this._trackAudio(currentAudio);
        ...
        currentAudio.addEventListener('loadedmetadata', () => {
            if (this._playSessionId !== currentSession) return;
        ...
        currentAudio.play().then(() => {
            ...
            console.warn('[AudioManager] Playlist error:', err);
        ...
        currentAudio.addEventListener('ended', () => advanceToNext(), { once: true });
    }

    // ──────────────────────────────────────────────────────────────────────────
    // CITY PLAYLIST HELPER
    // ──────────────────────────────────────────────────────────────────────────
    ...
    playCityPlaylist() {
        ...
        console.log('[AudioManager] Entering City: Starting playlist.');
    ...
    _fadeMP3(audioObj, targetVolume, duration, callback = null) {
        ...
        const volumeStep = (targetVolume - audioObj.volume) / steps;
    ...
    _nukeAudio(audioObj) {
        ...
        audioObj.load();
        if (this._activeTracks) this._activeTracks.delete(audioObj);
    }
    ...
    playNextNote() {
        ...
        this.currentStep++;
        if (this.currentStep >= track.pattern.length) this.currentStep = 0;
    }

    // ──────────────────────────────────────────────────────────────────────────
    // PROCEDURAL SOUND EFFECTS (Web Audio API synth sounds)
    // ──────────────────────────────────────────────────────────────────────────

    /**
     * @param {string} effect        - The sound to play
    ...
    playSound(effect, pitchVariance = 0.15, volVariance = 0.10) {
        ...
        // Combat synth effects also respect the combat gate
        const combatEffects = ['sword_clash', 'firelance', 'bomb', 'arrow', 'shield_block', 'charge', 'hit'];
        if (combatEffects.includes(effect) && wallClockNow < this._combatGateTime) return;
    ...
    /**
     * THE PRIMARY FIX FOR THE SANDBOX COORDINATE BUG.
     *
     * Must be called BEFORE deployArmy() / deployNavalArmy() on every battle type.
     * (naval AND land — previously it was only called on naval).
     *
     * What it does:
     *  1. Stops all currently playing SFX (both synth and MP3-pool).
     *  2. Arms a SHORT global gate (500ms) — blocks every sound while the engine
     *     repositions units from sandbox world-map coords to battle coords.
     *  3. Arms a LONG combat gate (3000ms) — blocks only melee/projectile/gunpowder
     *     sounds for the first 3 seconds. This prevents ghost proximity triggers
     *     if the deployment takes longer than 500ms on slow devices.
     *
     * Why 3 seconds for combat specifically?
     *  Units spawn at opposite ends of the battlefield and must march before
     *  actually reaching each other.  Any melee/projectile sound firing in the
     *  first 3 seconds is definitively a ghost trigger from stale coordinates.
     */
    clearAllSFXForBattle() {
        const now = Date.now();

        // Arm synth SFX gates
        this._sfxGateTime    = now + 500;    // 0.5s global synth block
        this._combatGateTime = now + 3000;   // 3s combat synth block

        // Stop all synth oscillators
        this.activeOscillators.forEach(osc => { try { osc.stop(); } catch(e) {} });
        this.activeOscillators = [];

        // ── Tell the MP3-pool SfxManager to go silent too ──────────────────
        // This is the key bridge: without this call, SfxManager (which manages
        // the real MP3 sound effects) has no idea a battle is starting and will
        // happily play melee sounds from the first frame.
        if (typeof SfxManager !== 'undefined') {
            // globalMs=500 → brief blackout while units reposition
            // combatMs=3000 → full 3s combat silence
            SfxManager.setSilence(500, 3000);
        }

        console.log('[AudioManager] Battle gate armed: global=500ms, combat=3000ms');
    }
}

// ============================================================================
// GLOBAL INSTANCE
// Single instantiation — no duplicate.  window.AudioManager is the same object.
// ============================================================================
const AudioManager = window.AudioManager = new AudioManagerSystem();