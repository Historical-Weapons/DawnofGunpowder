...
class Troop {
    constructor(name, role, isLarge, faction = "Generic") 
	{
		...
		// FIX: this.level was set but gainExperience() below actually reads/writes
		// this.experienceLevel, which was never initialized — expNeeded worked out
		// to (undefined * 1.0) = NaN, and "experience >= NaN" is always false, so
		// the level-up while-loop below could never fire. Troops have been silently
		// stuck at level 1 forever. Initializing it here is the whole fix.
		this.level = 1; this.experienceLevel = 1; this.experience = 0; this.morale = 20; this.maxMorale = 20; this.stamina = 100; this.health = 100; this.meleeAttack = 10; this.meleeDefense = 10;
        this.armor = ARMOR_TIERS.CLOTH; this.shieldBlockChance = 0; this.bonusVsLarge = 0;  this.isRanged = false; this.accuracy = 0; this.reloadSpeed = 0; this.missileBaseDamage = 0; this.missileAPDamage = 0;
...
// Rocket              | near instant
    ...
    // Rocket reload — SPEEDED UP AGAIN this session per direct request:
    // "wayyyyyy faster... all the ammo gets exhausted within 20
    // seconds just like fireworks" — corrected moments later to "2-10
    // seconds instead." The previous 42-tick (~700ms/shot) pace gave a
    // 20-round magazine a ~14 second burn time — too slow to read as a
    // firework volley either way. 9 ticks (~150ms/shot, at the game's
    // ~16.67ms/tick i.e. 60 ticks/sec) puts the full 20-round magazine
    // at 20 * 150ms = 3.0 seconds — comfortably inside the requested
    // 2-10s window and fast enough to feel like a firecracker string
    // rather than discrete gunshots. infscript.js's FIRE_CYCLE_MS (the
    // ignite->launch visual pulse) is kept matched 1:1 to this value —
    // see that constant's own comment for why they move together.
    if (name === "Rocket") return 9;
    ...
    // Cannon: split out from GUNNER on purpose — was sharing the hand
    // cannoneer's 300-tick (5s) reload, which reads as far too brisk for
    // a wagon gun needing a full swab/powder/shot/ram/aim cycle. 900
    // ticks (~15s at 60 ticks/sec) is 3x slower per direct request for
    // "much slower fire rate", keeping it clearly the slowest-firing
    // ranged unit in the roster.
    if (role === ROLES.MOUNTED_GUNNER) return 720; // 20% faster than the previous 900 (900*0.8=720), per direct request
